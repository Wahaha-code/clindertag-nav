...
void PoseEstimator::PnPSolver(MarkerInfo markers, vector<ModelInfo> reconstruct_model, CamInfo camera, PoseInfo& pose)
{
	...
	for (int j = 0; j < markers.cornerLists.size(); j++) {
		...
		const int featurePos =
			(j < markers.featurePos.size()) ? markers.featurePos[j] : -1;
		std::cout << "[PnP] marker=" << markers.markerID
		          << " featureIdx=" << j
		          << " featurePos=" << featurePos;
		if (j < markers.feature_ID_left.size())
			std::cout << " L=" << markers.feature_ID_left[j];
		if (j < markers.feature_ID_right.size())
			std::cout << " R=" << markers.feature_ID_right[j];
		std::cout << std::endl;
		for (int k = 0; k < 2; k++) {
			...
			int corner_id = markers.featurePos[j] * 8 + k;
			const Point3f& pt = reconstruct_model[ID].corners[corner_id];
			std::cout << "   cornerIdx2D=" << k << " -> 3D corner id "
			          << corner_id << " (" << pt.x << ", " << pt.y << ", "
			          << pt.z << ")" << std::endl;
		}
		for (int k = 4; k < 6; k++) {
	...
	if (!image_points.empty()) {
		...
		projectPoints(model_points, pose.rvec, pose.tvec, camera.Intrinsic, camera.distCoeffs, projected_points);

		double sum_sq = 0.0;
		for (size_t idx = 0; idx < projected_points.size(); ++idx) {
			const Point2f diff = projected_points[idx] - image_points[idx];
			sum_sq += diff.dot(diff);
		}
		pose.observationCount = projected_points.size();
		pose.reprojectionError = std::sqrt(sum_sq / projected_points.size());
	} else {
		pose.observationCount = 0;
		pose.reprojectionError = -1.0;
	}
}
