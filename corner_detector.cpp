...
using namespace std;
using namespace cv;
// 将 4 点整理成 TL,TR,BR,BL（按 y 分上下，再按 x 分左右）
static inline std::vector<cv::Point2f> toTLTRBRBL(std::vector<cv::Point2f> q) {
    CV_Assert(q.size()==4);
    // 按 y 升序
...
// 统一一个“条纹对”的 8 点顺序：
// 上排：TL(0),TR(1),BR(2),BL(3)
// 下排：BR(6),BL(7),TL(4),TR(5) —— 即“下半部分从右下角开始顺时针”
static inline void enforceEightCornerOrder(std::vector<cv::Point2f>& c){
    CV_Assert(c.size()==8);
    std::vector<cv::Point2f> q1(c.begin(), c.begin()+4);
    std::vector<cv::Point2f> q2(c.begin()+4, c.end());
    q1 = toTLTRBRBL(q1);
    q2 = toTLTRBRBL(q2);
    // 重新拼装为 [上排 TL,TR,BR,BL, 下排 BR,BL,TL,TR]
    c = { q1[0], q1[1], q1[2], q1[3],  q2[2], q2[3], q2[0], q2[1] };
}
...
featureInfo corner_detector::featureOrganization(std::vector<cv::Point2f> quad1,
    std::vector<cv::Point2f> quad2,
    cv::Point2f quad1_center,
    cv::Point2f quad2_center,
    float feature_angle)
{
float angle_max = 0, angle_min = 360;
int pos_quad1 = -1, pos_quad2 = -1;
...
for (int i = 0; i < 4; i++) {
if (std::min(360 - std::abs(angle_quad1[(i + 2) % 4] - feature_angle), std::abs(angle_quad1[(i + 2) % 4] - feature_angle))
+ std::min(360 - std::abs(angle_quad1[(i + 3) % 4] - feature_angle), std::abs(angle_quad1[(i + 3) % 4] - feature_angle)) < angle_min) {
angle_min = std::min(360 - std::abs(angle_quad1[(i + 2) % 4] - feature_angle), std::abs(angle_quad1[(i + 2) % 4] - feature_angle))
+ std::min(360 - std::abs(angle_quad1[(i + 3) % 4] - feature_angle), std::abs(angle_quad1[(i + 3) % 4] - feature_angle));
pos_quad1 = i;
}
if (std::min(360 - std::abs(angle_quad2[(i + 2) % 4] - feature_angle), std::abs(angle_quad2[(i + 2) % 4] - feature_angle))
+ std::min(360 - std::abs(angle_quad2[(i + 3) % 4] - feature_angle), std::abs(angle_quad2[(i + 3) % 4] - feature_angle)) > angle_max) {
angle_max = std::min(360 - std::abs(angle_quad2[(i + 2) % 4] - feature_angle), std::abs(angle_quad2[(i + 2) % 4] - feature_angle))
+ std::min(360 - std::abs(angle_quad2[(i + 3) % 4] - feature_angle), std::abs(angle_quad2[(i + 3) % 4] - feature_angle));
pos_quad2 = i;
}
}
for (int i = 0; i < 4; i++) {
...
Fea.feature_center = cv::Point2f((Fea.corners[0].x + Fea.corners[1].x + Fea.corners[4].x + Fea.corners[5].x) / 4.f,
(Fea.corners[0].y + Fea.corners[1].y + Fea.corners[4].y + Fea.corners[5].y) / 4.f);

// ★★★ 关键：强制 8 点顺序统一（上排 TL,TR,BR,BL；下排 BR,BL,TL,TR）
enforceEightCornerOrder(Fea.corners);
